...
## 2026-05-21 (#49) [论文] 图表阶段收工 — 确定图号、写图注、清理目录

**做了什么：**
- 参考优秀论文（洪涝应急决策系统）的图表结构，确定第3章只需3张图：图3-1 物理架构、图3-2 软件架构、图3-3 系统整体流程图
- 用户自画的图4-1（STM32引脚连接）和图4-2（三级输出定义）决定弃用——对应位置已有表4-2、表4-3，加图是重复
- 写了5张图的图注（论文/图注-插入用.txt），后精简为3张
- 写了图3-3系统整体流程图的ASCII草图供用户参考
- 清理论文目录：删除旧版终稿4个、Word临时文件2个、组装脚本3个、node_modules、旧流程图3个、其他临时产物约7个，共约20项

**关键决策：**
- 从原计划14张图简化为3张核心图，与优秀论文的简洁风格对齐——优秀论文第3章也只有4张架构级图，所有详细流程图放第4章
- 图表不是越多越好，表格能说清楚的就不要加图

**涉及文件：** `论文/图注-插入用.txt`（新建）、`论文/`（清理）、`F:/下载临时/AI临时/图3-3-系统整体流程图-草图.txt`

**下一步：** 3张图插入降重版.docx → 更新正文引用 → 终稿定稿；终期答辩PPT

---

## 2026-05-20 (#48) [教学] 代码教学8课 — 答辩准备

**做了什么：**
- 生成 8 个教学文件（`docs/代码教学-第N课-*.md`），覆盖从摄像头到硬件的完整链路
- 每课聚焦一个模块：系统全景→检测引擎→报警防抖→联动决策→MQTT通信→API与前端→数据库与通知→硬件固件
- 每课结尾附"答辩时怎么说"一句话模板

**涉及文件：** `docs/代码教学.md`（入口）、`docs/代码教学-第1~8课-*.md`（8个）

**下一步：** 用户自学，有问题随时问

---

## 2026-05-20 (#47) [论文] 写作经验总结更新 + 子对话流程图审查 + 项目清理

**做了什么：**
- `论文写作经验总结.md` 大幅更新：第五节新增 5.1 结构降重（抽象层级提升对照表）、5.3 格式降重（去加粗/去强调/去分隔线）；第四节新增模板文本框提取方法 + 11项格式速查表；第七节新增14条踩坑记录
- 审查子对话生成的8张Mermaid图——首版标注过细（函数名/引脚号/具体数值），子对话修复后：图4-2（改为时序图）和图4-3（改为锁执行时序）改善明显；图3-2两行分组布局混乱、拓扑信息丢失；用户决定自己手画
- 项目清理：删除 v3/v5/v7/v8/v10 旧版docx、`_tmp_merged.docx`、`~$` Word锁文件
- 新增反馈记忆 `feedback_sync_reminder.md`（每次commit后主动提醒同步进度）

**关键决策：**
- Mermaid图不适合学术论文排版——纵向流程图高宽比失控、布局控制弱，推荐用户直接在早期.drawio模板上改
- 图的降重标准和文字一样：去掉函数名、引脚号、具体数值参数

**涉及文件：** `论文/论文写作经验总结.md`（大幅更新）、`论文/流程图/`（8张.mmd+.png+.drawio）、`CLAUDE.md`、`docs/PROGRESS.md`

**下一步：** 用户手画4张图 → 图片插入终稿 → 终稿定稿

---

## 2026-05-20 (#46) [论文] 刘晖论文 — 18张图按工作流全部重生成

**做了什么：** 按 `图表生成工作流.md` 标准流程（.mmd → mmdc渲染PNG → Python生成.drawio）为刘晖防汛论文重新生成全部18张图。内容对齐 `终稿-修复版.docx`，全部应用 §5 AIGC降重规则（去除函数名/引脚号/端口号/具体数值/代码变量/协议细节）。

**涉及文件：**
- 新增：`刘晖/论文/*.mmd`（18个Mermaid源文件）、`刘晖/论文/*.png`（18张高清渲染图）
- 覆盖：`刘晖/论文/*.drawio`（18个draw.io可编辑版本）
- 新增：`刘晖/论文/generate_drawio.py`（批量生成脚本，可删除）

**下一步：** 刘晖验证图内容正确性 → 用户手动精修.drawio → 插入终稿

---

## 2026-05-20 (#45) [论文] 格式精修四连 — 去加粗/除强调/标题重组/清分隔线

**做了什么：** 四轮格式修复迭代（v8→v11）：
- v8: 所有标题 bold=True 移除（assemble_thesis.py + merge_thesis.py），黑体本身就是粗重字体，bold=True 造成仿粗体双重加粗
- v9: 全文 `**...**` 加粗强调标记清除（第1/3/4/5章共364个星号），仅保留英文 Keywords 加粗（模板 TEXTBOX 13 要求）
- v10: 第4章标题层级重组——去掉空壳 `## 4.1 系统实现`，4.1.1~4.1.3 升级为 4.1~4.3，`#### 1~8` 升级为 `### 4.2.1~4.2.8` 带编号，消除无编号四级标题
- v11: 删除第1/4/5章末尾 `---` 分隔线（无实际排版意义）
- 封面字体两次修正：标签列改仿宋三号（模板 TEXTBOX 2），值列改 Times New Roman 三号

**关键决策：**
- `**...**` 加粗强调被认定为 AI 写作典型特征，全文清除
- 章节结构对称为重：第4章原只有 4.1（测试被拆为第5章），标题升级后4.1/4.2/4.3 三节并列
- 版本递增约定确立：每次修改 → 新版本号 → 删旧版，当前 v11

**涉及文件：** `论文/第1/3/4/5章*.md`、`论文/assemble_thesis.py`、`论文/merge_thesis.py`、`论文/generate_cover.cjs`、`论文/终稿-标准格式-v11.docx`

**下一步：** 用户手动重画4张图 → 全部图片插入终稿 → 终稿定稿
